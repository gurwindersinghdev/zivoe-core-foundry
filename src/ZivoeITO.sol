...
contract ZivoeITO is Context {
    ...
    /// @dev Stablecoin(s) allowed for juniorDeposit() or seniorDeposit().

    uint256 public end;

    /// @dev The unix when the ITO ends (airdrop is claimable).
    ...
    /// @dev Tracks if an account has claimed their airdrop.

    mapping(address => uint256) public juniorCredits;

    /// @dev Tracks $pZVE (credits) from juniorDeposit().
    mapping(address => uint256) public seniorCredits;
    /// @dev Tracks $pZVE (credits) from seniorDeposit().
    ...
    event JuniorDeposit(
        address indexed account, address indexed asset, uint256 amount, uint256 credits, uint256 trancheTokens
    );
    ...
    function claimAirdrop(address depositor)
        external
        returns (uint256 zSTTClaimed, uint256 zJTTClaimed, uint256 ZVEVested)
    {
        ...
        uint256 seniorCreditsOwned = seniorCredits[depositor]; // senior credits = 3
        uint256 juniorCreditsOwned = juniorCredits[depositor]; // junior credits = 1
        ...
        uint256 middle = IERC20(IZivoeGlobals_ITO(GBL).ZVE()).totalSupply() / 20; // 100/20 = 5% distribution total zivoe tokens
    ...
    function depositBoth(uint256 amountSenior, address assetSenior, uint256 amountJunior, address assetJunior)
        external
    {
