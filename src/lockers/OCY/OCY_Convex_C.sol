...
contract OCY_Convex_C is ZivoeLocker, ReentrancyGuard {
    ...
    function pushToLocker(address asset, uint256 amount, bytes calldata data) external override onlyOwner {
        require(asset == PYUSD || asset == USDC, "OCY_Convex_C::pushToLocker() asset != PYUSD && asset != USDC");
        IERC20(asset).safeTransferFrom(owner(), address(this), amount);

        (uint256 _min_mint_amountBP) = abi.decode(data, (uint256));
        ...
        uint256 balCurveBasePoolToken = IERC20(curveBasePoolToken).balanceOf(address(this));
    ...
    function pullFromLocker(address asset, bytes calldata data) external override onlyOwner {
        ...
        (uint256 _bp_min0, uint256 _bp_min1) = abi.decode(data, (uint256, uint256));

        // Burn BasePool Tokens
        uint256[] memory _min_amounts_bp = new uint256[](2);
