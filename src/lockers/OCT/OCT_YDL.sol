...
contract OCT_YDL is ZivoeLocker, ZivoeSwapper, ReentrancyGuard {
    ...
    event AssetConvertedForwarded(
        address indexed asset, address indexed distributedAsset, uint256 amountFrom, uint256 amountTo
    );
    ...
    function convertAndForward(address asset, bytes calldata data) external nonReentrant {
        require(IZivoeGlobals_OCT_YDL(GBL).isKeeper(_msgSender()), "OCT_YDL::convertAndForward !isKeeper(_msgSender())");
        ...
        uint256 balDistributedAsset = IERC20(distributedAsset).balanceOf(address(this));
