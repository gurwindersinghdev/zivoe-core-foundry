...
contract OCL_ZVE is ZivoeLocker, ReentrancyGuard {
    ...
    /// @dev Address for the Factory (Uniswap v2 or Sushi).
    address public immutable pairAsset;
    /// @dev ERC20 that will be paired with $ZVE for Sushi pool.
    address public immutable router;
    /// @dev Address for the Router (Uniswap v2 or Sushi).

    address public OCT_YDL;

    /// @dev Facilitates swaps and forwards distributedAsset() to YDL.

    uint256 public basis;

    /// @dev The basis used for forwardYield() accounting.
    uint256 public compoundingRateBIPS = 5000;
    /// @dev The % of returns to retain, in BIPS.
    uint256 public nextYieldDistribution;
    /// @dev Determines next available forwardYield() call.
    ...
    function pushToLockerMulti(address[] calldata assets, uint256[] calldata amounts, bytes[] calldata data)
        external
        override
        onlyOwner
        nonReentrant
