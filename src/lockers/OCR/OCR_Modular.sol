...
contract OCR_Modular is ZivoeLocker, ReentrancyGuard {
    ...
    struct Request {
        address account;
        /// @dev The account making the request.
        uint256 amount;
        /// @dev The amount of the request ($zSTT or $zJTT).
        uint256 unlocks;
        /// @dev The timestamp after which this request may be processed.
        bool seniorElseJunior;
    }
    /// @dev The tranche this request is for (true = Senior, false = Junior).

    address public immutable GBL;

    /// @dev The ZivoeGlobals contract.
    address public immutable stablecoin;
    /// @dev The stablecoin redeemable in this contract.

    uint256 public epoch;

    /// @dev The timestamp of current epoch.
    uint256 public epochDiscountJunior;
    /// @dev Redemption discount for $zJTT (junior tranche).
    uint256 public epochDiscountSenior;
    /// @dev Redemption discount for $zSTT (senior tranche).
    uint256 public redemptionsAllowedJunior;
    /// @dev Redemptions allowed for $zJTT (junior tranche).
    uint256 public redemptionsAllowedSenior;
    /// @dev Redemptions allowed for $zSTT (senior tranche).
    uint256 public redemptionsFeeBIPS;
    /// @dev Fee for redemptions (in BIPS).
    uint256 public redemptionsQueuedJunior;
    /// @dev Redemptions queued for $zJTT (junior tranche).
    uint256 public redemptionsQueuedSenior;
    /// @dev Redemptions queued for $zSTT (senior tranche).
    uint256 public requestCounter;
    /// @dev Increments with new requests.
    ...
    function pushToLocker(address asset, uint256 amount, bytes calldata data)
        external
        override
        _tickEpoch
        onlyOwner
    ...
    function destroyRequest(uint256 id) external _tickEpoch nonReentrant {
        require(
            requests[id].account == _msgSender(), "OCR_Modular::destroyRequest() requests[id].account != _msgSender()"
        );
    ...
    function processRequest(uint256 id) external _tickEpoch nonReentrant {
        ...
        uint256 stablecoinBalance =
            IZivoeGlobals_OCR(GBL).standardize(IERC20(stablecoin).balanceOf(address(this)), stablecoin);
    ...
    function tickEpoch() public {
        while (block.timestamp >= epoch + 14 days) {
            ...
            emit EpochTicked(
                epoch, redemptionsAllowedJunior, redemptionsAllowedSenior, epochDiscountJunior, epochDiscountSenior
            );
        }
    }
    ...
    function updateRedemptionsFeeBIPS(uint256 _redemptionsFeeBIPS) external _tickEpoch {
        ...
        require(_redemptionsFeeBIPS <= 2000, "OCR_Modular::updateRedemptionsFeeBIPS() _redemptionsFeeBIPS > 2000");
