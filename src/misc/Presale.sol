...
contract Presale is OwnableLocked, ReentrancyGuard {
    ...
    /// @dev Chainlink oracle for ETH.

    address public treasury;

    /// @dev The treasury for the presale.

    uint256 public pointsFloor = 250;

    /// @dev Minimum amount of points earnable, per stablecoin deposit.

    uint256 public pointsCeiling = 5000;

    /// @dev Maximum amount of points earnable, per stablecoin deposit.

    uint256 public presaleStart;

    /// @dev The timestamp at which the presale starts.

    uint256 public presaleDuration = 21 days;

    /// @dev The amount of time presale lasts.

    mapping(address => bool) public stablecoinWhitelist;

    /// @dev Whitelist for stablecoins.

    mapping(address => uint256) public points;

    /// @dev Track points per user.
    ...
    event StablecoinDeposited(
        address indexed depositor, address indexed stablecoin, uint256 amount, uint256 pointsAwarded
    );
    ...
    /// @param  pointsAwarded The amount of points awarded to depositor.
    event ETHDeposited(address indexed depositor, uint256 amount, uint256 oraclePrice, uint256 pointsAwarded);
    ...
    function oraclePrice() public view returns (uint256 price) {
        price = uint256(IPresale_Oracle(oracle).latestAnswer());
    }
    ...
    function pointsAwardedETH(uint256 amount) public view returns (uint256 pointsAwarded, uint256 priceEth) {
        ...
        pointsAwarded = ((priceEth * amount) / (10 ** 8))
            * (pointsFloor + (pointsCeiling - pointsFloor) * (21 days - (block.timestamp - presaleStart)) / 21 days);
    }
    ...
    function depositStablecoin(address stablecoin, uint256 amount) public {
        ...
        // Enforce minimum 10
        uint256 checkAmount = standardize(stablecoin, amount);
