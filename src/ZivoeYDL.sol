...
contract ZivoeYDL is Context, ReentrancyGuard {
    ...
    function distributeYield() external nonReentrant {
        ...
        require(
            block.timestamp >= lastDistribution + daysBetweenDistributions * 86400, // 30 days  * 86400
            "ZivoeYDL::distributeYield() block.timestamp < lastDistribution + daysBetweenDistributions * 86400"
        );

        // Calculate protocol earnings.
        uint256 earnings = IERC20(distributedAsset).balanceOf(address(this)); // asset bal = 100
        uint256 protocolEarnings = protocolEarningsRateBIPS * earnings / BIPS;        // 2000 + earn(100) / 10000 = 20%
        uint256 postFeeYield = earnings.floorSub(protocolEarnings);  // 100 - 2000 = 1900
    ...
    function updateTargetAPYBIPS(uint256 _targetAPYBIPS) external {
        ...
        targetAPYBIPS = _targetAPYBIPS;  //1000
    }
    ...
    function updateTargetRatioBIPS(uint256 _targetRatioBIPS) external {
        ...
        targetRatioBIPS = _targetRatioBIPS;  //22000
    }
    ...
    function earningsTrancheuse(uint256 yP, uint256 yD) public view returns (
        ...
        for (uint256 i = 0; i < protocolRecipients.recipients.length; i++) {
            protocol[i] = protocolRecipients.proportion[i] * yP / BIPS;                 // 100 * 20 / 10000 = 0.2%
        }
