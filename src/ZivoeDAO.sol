...
interface ILocker_DAO {
    ...
    /// @param  amounts The amounts of "assets" to migrate, corresponds to "assets" by position in array.
    /// @param  data    Accompanying transaction data.
    function pullFromLockerMultiPartial(address[] calldata assets, uint256[] calldata amounts, bytes[] calldata data)
        external;
    ...
    /// @param  data        Accompanying transaction data.
    function pushToLockerMultiERC721(address[] calldata assets, uint256[] calldata tokenIds, bytes[] calldata data)
        external;
    ...
    /// @param  tokenIds    The IDs of the NFTs to migrate.
    /// @param  data        Accompanying transaction data.
    function pullFromLockerMultiERC721(address[] calldata assets, uint256[] calldata tokenIds, bytes[] calldata data)
        external;
    ...
    /// @param  data    Accompanying transaction data.
    function pushToLockerERC1155(address asset, uint256[] calldata ids, uint256[] calldata amounts, bytes calldata data)
        external;
...
contract ZivoeDAO is ERC1155Holder, ERC721Holder, OwnableLocked, ReentrancyGuard {
    ...
    address public immutable GBL;

    /// @dev The ZivoeGlobals contract.
    ...
    /// @param  data    Accompanying transaction data.
    function pullPartial(address locker, address asset, uint256 amount, bytes calldata data)
        external
        onlyOwner
        nonReentrant
    {
    ...
    /// @param  data    Accompanying transaction data.
    function pushMulti(address locker, address[] calldata assets, uint256[] calldata amounts, bytes[] calldata data)
        external
        onlyOwner
        nonReentrant
    {
        require(
            IZivoeGlobals_DAO(GBL).isLocker(locker), "ZivoeDAO::pushMulti() !IZivoeGlobals_DAO(GBL).isLocker(locker)"
        );
    ...
    /// @param  data    Accompanying transaction data.
    function pullMulti(address locker, address[] calldata assets, bytes[] calldata data)
        external
        onlyOwner
        nonReentrant
    {
    ...
    /// @param  data    Accompanying data for the transaction.
    function pushERC721(address locker, address asset, uint256 tokenId, bytes calldata data)
        external
        onlyOwner
        nonReentrant
    {
        require(
            IZivoeGlobals_DAO(GBL).isLocker(locker), "ZivoeDAO::pushERC721() !IZivoeGlobals_DAO(GBL).isLocker(locker)"
        );
    ...
    /// @param  data    Accompanying data for the transaction.
    function pullERC721(address locker, address asset, uint256 tokenId, bytes calldata data)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function pushERC1155(
        ...
        require(
            IZivoeGlobals_DAO(GBL).isLocker(locker), "ZivoeDAO::pushERC1155() !IZivoeGlobals_DAO(GBL).isLocker(locker)"
        );
        require(ILocker_DAO(locker).canPushERC1155(), "ZivoeDAO::pushERC1155() !ILocker_DAO(locker).canPushERC1155()");
    ...
    function pullERC1155(
        ...
        require(ILocker_DAO(locker).canPullERC1155(), "ZivoeDAO::pullERC1155() !ILocker_DAO(locker).canPullERC1155()");
